/*
	...
	Log: 
		BugFix：自动播放状态时，鼠标从showNav元素上移走后，可能无法恢复自动播放的Bug;
		AddLog: 新增了实例对象可用的pause()、resume()方法，用于自动播放的中断、继续

	Update: 2015-07-30
	Log: BugFix，解决dir: "F3"时默认显示banner图有误问题、以及IE浏览器下循环切换时的Bug、解决快速点击时响应慢的问题

	Update: 2015-07-27
	Log: BugFix，解决有自动滚动与用户控制滚动事件冲突造成的滚动混乱问题
	
	Update: 2015-07-17
	Log: 添加fadeIn、fadeOut切换动画: dir: "F3"

	Demo:
	$(".productshow").Xslider({
			unitDisplayed: 1,//可视的单位个数
			numtoMove: 1,
			dir: "H",//水平移动还是垂直移动       默认H为水平移动，传入V则表示垂直移动,传入F3则表示渐变切换（注意是大写字母）;
   ...
   pause: function ();//中止自动播放
   resume: function ();//继续自动播放
