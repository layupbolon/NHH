...
namespace NHH.WebSite.Image.Controllers
{
    ...
    public class ComplaintController : NHHController
    {
        ...
        public JsonResult Upload(int projectId)
        {
            ...
            string fileName = string.Format("{0}-{1}{2}", date.ToString("yyMMddHHmmss"), random.Next(0, 9), fileExt);
            ...
            result.Result = true;
            result.FilePath = string.Format("Complaint/Original/{0}/{1}/{2}/{3}/{4}", projectId, date.Year, date.Month, date.Day, fileName);
        ...
        {
            string path = HttpContext.Server.MapPath("/Complaint/");
            //文件尺寸
            if (fileSize > 0)
            {
