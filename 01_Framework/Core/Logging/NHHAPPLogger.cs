...
namespace NHH.Framework.Logging
{
    ...
    public class NHHAPPLogger : IAPPLogger
    {
        ...
        #region Context
        private APPLoggingContext m_Context;
        /// <summary>
        /// 日志上下文对象
        /// </summary>
        protected APPLoggingContext Context
        {
            get
            {
                if (m_Context == null)
                {
                    ...
                    m_Context = string.IsNullOrEmpty(dbname) ? new APPLoggingContext() : new APPLoggingContext(dbname);
                }
                return m_Context;
            }
        }
        ...
        protected int Level
        {
            get
            {
                if (m_Level == -1)
                {
                    var lname = (ParamManager.GetStringValue("logging:app_level") ?? "DEBUG").ToUpper();
    ...
    #region APPLoggingContext
    /// <summary>
    /// 系统日志记录数据库上下文
    /// </summary>
    public class APPLoggingContext : DbContext
    {
        public APPLoggingContext()
            : base()
        {

        }
        public APPLoggingContext(string nameOrConnectionString)
